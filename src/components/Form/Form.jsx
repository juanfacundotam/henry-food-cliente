...
const Form = ({ buttonName, handleServerFunction, withRecipe }) => {
  ...
  const printInstruction = () => {
    ...
    const newInstructions = arrayInstructions.map((elem, index) => {
      return (
        <div key={index} className={style.divItemInstructions}>
  ...
  const printCheckDiet = () => {
    const printDiets = allDiets?.map((elem, index) => {
      return (
        <div key={index} className={style.divItemDiet}>
